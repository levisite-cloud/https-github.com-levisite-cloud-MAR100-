...
export const DashboardView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      <div className="bg-gradient-to-r from-zinc-900 via-zinc-850 to-zinc-900 border border-emerald-500/30 rounded-2xl p-4 sm:p-5 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3.5 w-full md:w-auto">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-sm font-black text-zinc-100">Atendimento Automatizado via WhatsApp</h3>
              <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 flex items-center gap-1">
                ...
                {botStatus === 'connected' ? 'Robô Conectado ao Celular' : 'Pronto para Pareamento'}
              </span>
            </div>
            <p className="text-xs text-zinc-400 mt-0.5">
              {conversas.length} conversas ativas no momento • O robô calcula orçamentos e transfere leads para o Kanban em 1 clique.
            </p>
          </div>
        </div>
        ...
        <button
          onClick={() => setActiveView('bot')}
          className="w-full md:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-black shadow-md shadow-emerald-600/20 transition-all active:scale-95 cursor-pointer shrink-0"
        >
          <MessageSquare className="w-4 h-4" />
          <span>Abrir Central do Robô</span>
